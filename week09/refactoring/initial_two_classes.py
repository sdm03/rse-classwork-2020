...
class Group:
    ...
    def number_of_connections(self, name):
        ...
        if self.contains(name):
            ...
            raise Exception('Group does not contain name')
    ...
    def connect(self, name1, name2, relation, reciprocal=True):
        ...
        """
        if self.contains(name1) and self.contains(name2):
            if name1 not in self.connections.keys():
                self.connections[name1] = {name2: relation}
            else:
                self.connections[name1].update({name2: relation})
            if reciprocal:
                if name2 not in self.connections.keys():
                    self.connections[name2] = {name2: relation}
                else:
                    self.connections[name2].update({name1: relation})
        else:
            raise Exception('Group does not contain both names')
